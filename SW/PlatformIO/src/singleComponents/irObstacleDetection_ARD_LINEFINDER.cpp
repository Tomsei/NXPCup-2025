...
namespace SingleComponent {
    ...
    void runArdLineFinder() {
        ...
        if (valSensor1 == HIGH && blackSensor1) {
            CONSOLE.println("ARD Linefinder 1: Black to White");
            blackSensor1 = false;
        } else if (valSensor1 == LOW && !blackSensor1) {
            CONSOLE.println("ARD Linefinder 1: White to Black");
            blackSensor1 = true;
        }

        if (valSensor2 == HIGH && blackSensor2) {
            CONSOLE.println("ARD Linefinder 2: Black to White");
            blackSensor2 = false;
        } else if (valSensor2 == LOW && !blackSensor2) {
            CONSOLE.println("ARD Linefinder 2: White to Black");
            blackSensor2 = true;
        }
    }
}
